...
public class PercentageDataAdapter extends AdapterOperations<Percentage, PercentageData, Long, PercentageDataRepository> implements PercentageRepository {
    ...
    public Percentage savePercentage(Percentage percentage) {
        PercentageData percentageData = this.repository.saveAndFlush(percentageMapper.percentageToPercentageData(percentage));
        return percentageMapper.percentageDataToPercentage(percentageData);
    }

    @Override
    public Optional<Percentage> getLastPercentageInsert() {

        Optional<PercentageData> p = this.repository.findFirstByOrderByIdDesc();

        if(!p.isPresent()){
            return Optional.empty();
        }

        return Optional.of(percentageMapper.percentageDataToPercentage(p.get()));
    }
